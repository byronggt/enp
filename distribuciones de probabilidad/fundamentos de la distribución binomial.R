...
# En un examen de selección múltiple, con 10 preguntas con 4 posibles respuestas
# y una sola de ellas como correcta
# Valores de la distribución binomial para n=10 p=0.25 q=0.75

tabla.p=data.frame(Probability=dbinom(0:10, size=10, prob=0.25))
rownames(tabla.p) <- 0:10 
print(tabla.p)

# ¿Cuál es la probabilidad de responder ninguna correcta?
# P(X=0)
P0<-dbinom(0,10,0.25); P0 # Usar dbinom para valores puntuales de probabilidad

# ¿Cuál es la probabilidad de responder de forma correcta 6 preguntas o menos?
# P(X<=6)
...
# ¿Cuál es la probabilidad de responder de forma correcta 7 preguntas o más?
# P(X<=6)
# Si es cola derecha debe ser un valor abajo del punto de interés
